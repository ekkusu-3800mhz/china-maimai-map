...
<html lang="zh_CN">
    ...
    <body>
        ...
        <script type="text/javascript">$(document).ready(function(){var d="{{$apiUrl}}";var g=new BMap.Map("map");var b=new BMap.Point(114.313886,30.601948);var a=new BMap.Geolocation();var f=new BMap.Geocoder();g.centerAndZoom(b,6);g.enableScrollWheelZoom(true);g.addControl(new BMap.ScaleControl());g.addControl(new BMap.NavigationControl());g.addControl(new BMap.GeolocationControl({offset:new BMap.Size(15,50)}));function c(j,i){if(i.count>1){var m="__IMG__/map-icon-multi.png"}else{var m="__IMG__/map-icon-single.png"}var o=new BMap.Icon(m,new BMap.Size(40,40),{anchor:new BMap.Size(20,20)});var k=new BMap.Marker(j,{icon:o});g.addOverlay(k);var n={width:250,height:110,title:i.title};var l=new BMap.InfoWindow(i.content,n);k.addEventListener("click",function(){g.openInfoWindow(l,j)})}function e(j,i){j.done(function(k){if(k.status==200){g.clearOverlays();var m=k.data.rows;var l=false;if(m.length>1){l=true}m.forEach(function(n){f.getPoint(n.address,function(p){if(p){var o={title:n.name,content:'<span style="font-size: 11px; color: #666;">店铺地址：'+n.address+'</span><br><span style="font-size: 11px; color: #666;">机台数量：'+n.count+"组</span>",count:n.count};if(n.lnglat!=null){p=new BMap.Point(n.lnglat.lng,n.lnglat.lat)}c(p,o);if(i){if(l){g.centerAndZoom(p,6)}else{g.centerAndZoom(p,17)}}}},n.province)})}else{alert("发生请求错误，请刷新页面重试")}});j.fail(function(){alert("发生请求错误，请刷新页面重试")});j.always(function(){$("#loader").fadeOut("fast");a.getCurrentPosition(function(k){if(this.getStatus()==BMAP_STATUS_SUCCESS){g.panTo(k.point)}})})}var h=$.get(d+"?query={{$location}}");if("{{$location}}"==""){e(h,false)}else{e(h,true)}$("#home").on("click",function(j){j.preventDefault();$("#loader").fadeIn("fast");var i=$("#query").val();h=$.get(d);e(h,false)});$("#search-form").on("submit",function(j){j.preventDefault();$("#loader").fadeIn("fast");var i=$("#query").val();h=$.get(d+"?query="+i);e(h,true)})});</script>
    </body>
</html>
